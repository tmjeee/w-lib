...
import { JsEventEmitter, EventEmitter, createLoadingState, FsmState, Fsm } from '../src';
...
const exampleLoadingState = async () => {
  const loadingState = createLoadingState();
  const state1 = loadingState.is('evt1');
  console.log('state1', state1);
  await loadingState.withLoading('evt1', async ()=>{
    console.log('doing loading');
    await setTimeout(async ()=>{}, 1000);
  });
  const state2 = loadingState.is('evt1');
